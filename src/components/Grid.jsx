...
export default  function Grid() {
    ...
    function handleSave(index, data) { 
        ...
        const newObj = {
            ...
            [index]: {
                ...data,  
                isComplete: !!data.isComplete || !!savedWorkouts?.[index]?.
                isComplete   
            }
        }

        setSavedWorkouts(newObj)
        localStorage.setItem('regimen', JSON.stringify(newObj))
        setSelectedWorkout(null)
    }


    function handleComplete(index,data){
        //complete a workout, modifies the completed status
        const newObj = { ...data } 
    ...
    useEffect(() => {
        ...
        if (localStorage.getItem('regimen')){
            savedData = JSON.parse(localStorage.getItem('regimen'))
        }
                    ...
                    return (
                        <button onClick = {() => {
                            ...
                            <div className = 'plan-card-header'>
                                {/*Appending 0 in front of the workout index*/}
                                <p>Day {dayNum}</p>
                                    </div>
                                    {isLocked ? (
